...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # (1) 
        # first transfrom the tokens (that are just descrete ~30k integers for each word) into embeddings 
        #       that their length and orientation themselves have meaning
        # combines (adds) the input_ids (words converted to integers), 
        #                     position_ids (positions), 
        #                     and token_type_ids (which segment each token belongs, eg the question or the context) into a single embedding output 
        # it is learnable
        # each word in a sentence from an integer to a vector of length=hidden_size (eg 768)
        ...
        # (2)
        # encode the embeddings, the encoder that uses attention, the shape won't change
